...
export async function insertLibrary(us_id, formData, setBtnloading, Alert) {
	try {
		...
		const docRef = await addDoc(collection(db, "library"), {
			...
			li_status: "Active",
			li_schoolID: formData.li_schoolID || "",
			li_name: formData.li_name || "",
			li_schoolname: formData.li_schoolname || "",
			li_email: formData.li_email || "",
			li_phone: formData.li_phone || "",
			li_description: formData.li_description || "",
			li_address: formData.li_address || "",
			li_latlng: formData.li_latlng || "",
			li_photoURL: li_photoURL,
